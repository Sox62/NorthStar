...
const EMPTY: LocalStore = { version: 2, transactions: [], positions: [], cashAccounts: [], snapshots: [], imports: [] };

async function readStore(): Promise<LocalStore> {
  try {
    const parsed = JSON.parse(await readFile(DATA_FILE, "utf8")) as LocalStore;
    return parsed.version === 2 ? parsed : structuredClone(EMPTY);
  } catch (error) {
...
function recalculateIbkrPositions(store: LocalStore, ownerType: OwnerType, accountKey: string) {
  ...
  for (const [instrumentKey, transactions] of grouped) {
    ...
    store.positions.push({
      ...
      name: latest.description || latest.symbol,
      exchange: latest.exchange,
      currency: latest.currency,
      assetClass: classifyAsset(latest.symbol, latest.description || ""),
      quantity,
      lastPrice: latest.closePrice ?? latest.price ?? null,
      averageCostAud: quantity ? safeCost / quantity : 0,
      costAud: safeCost,
      marketValueAud: safeCost,
      dayGainAud: 0,
      pnlAud: 0,
      pnlPercent: 0,
...
function captureSnapshot(store: LocalStore, ownerType: OwnerType) {
  ...
  const snapshot: Snapshot = {
    ...
    marketValue: positions.reduce((sum, position) => sum + position.marketValueAud, 0),
    cashValue: cash.reduce((sum, account) => sum + account.balanceAud, 0),
    netContributions: 0,
  };
...
function dashboardFromStore(store: LocalStore, scope: Scope): DashboardData {
  ...
  const positions = store.positions.filter(position => !ownerType || position.ownerType === ownerType);
  ...
  const allocations = [...allocationAmounts.entries()]
    .map(([name, amount]) => ({ name, amount, value: totalValue ? amount / totalValue * 100 : 0 }))
  ...
  const performance = [...daily.entries()].sort(([a], [b]) => a.localeCompare(b)).slice(-90).map(([date, item]) => {
    ...
    return {
      date: new Intl.DateTimeFormat("en-AU", { day: "numeric", month: "short" }).format(new Date(`${date}T12:00:00Z`)),
  ...
  const accountRows = imports.map(record => ({
  ...
  for (const account of cashAccounts) accountRows.push({
  ...
  return {
    ...
    dailyMovement,
    totalReturn,
    totalReturnPercent: totalCost ? totalReturn / totalCost * 100 : 0,
    holdings,
    allocations,
    performance,
...
export class LocalStorageAdapter implements StorageAdapter {
  async importIbkr(transactions: ImportedTransaction[], ownerType: OwnerType): Promise<ImportResult> {
    ...
    recalculateIbkrPositions(store, ownerType, accountKey);
    ...
    if (importRecord) {
      ...
      importRecord.recordCount = store.transactions.filter(transaction => transaction.ownerType === ownerType && transaction.broker === "IBKR" && transaction.accountKey === accountKey).length;
    } else {
      store.imports.push({ id: randomUUID(), source: "IBKR", ownerType, importedAt: new Date().toISOString(), recordCount: imported, accountKey });
    }
    ...
    return {
      source: "IBKR",
      ownerType,
      accountKey: maskAccount(accountKey),
      imported,
      duplicates,
      positions: store.positions.filter(position => position.ownerType === ownerType && position.broker === "IBKR" && position.accountKey === accountKey).length,
      storageMode: "local-file",
    };
  }
  ...
  async importDirectshares(positions: OpeningPosition[], ownerType: OwnerType): Promise<ImportResult> {
    ...
    for (const position of positions) {
      store.positions.push({
        ...
        broker: "Directshares",
        accountKey,
        instrumentKey: `Directshares:${position.symbol}:${position.exchange}`,
        ...
        averageCostAud: position.averageCostAud,
    ...
    if (importRecord) {
      importRecord.importedAt = new Date().toISOString();
      importRecord.recordCount = positions.length;
    } else {
      store.imports.push({ id: randomUUID(), source: "Directshares", ownerType, importedAt: new Date().toISOString(), recordCount: positions.length, accountKey });
    }
    captureSnapshot(store, ownerType);
    await writeStore(store);
  ...
  async upsertCashAccount(input: Omit<CashAccount, "id" | "updatedAt" | "balanceAud"> & { id?: string }) {
    ...
    const account: CashAccount = {
      ...
      fxRateToAud: input.fxRateToAud,
      balanceAud: input.balance * input.fxRateToAud,
  ...
  async dashboard(scope: Scope) {
    return dashboardFromStore(await readStore(), scope);
  }
}
