...
export const fundamentalResearchDrafts = pgTable("fundamental_research_drafts", {
  id: uuid("id").defaultRandom().primaryKey(),
  symbol: text("symbol").notNull(),
  name: text("name"),
  primaryMetal: text("primary_metal"),
  ...
  extractor: text("extractor").notNull().default("manual"),
  ...
  index("fundamental_research_drafts_symbol_idx").on(table.symbol),
  index("fundamental_research_drafts_status_idx").on(table.status),
]);
