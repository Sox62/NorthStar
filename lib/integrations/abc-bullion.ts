...
export function parseAbcPlatinumPricePage(html: string, retrievedAt = new Date().toISOString()): PlatinumPrice {
  ...
  if (index < 0) throw new Error("SouthernStar could not find ABC Bullion's 1 kg platinum product on the pricing page.");
  ...
  if (values.length < 2) throw new Error("SouthernStar could not read ABC Bullion's platinum selling and buying prices.");
...
export async function fetchAbcPlatinumPrice(): Promise<PlatinumPrice> {
  ...
  try {
    const response = await fetch(SOURCE_URL, {
      cache: "no-store",
      headers: {
        "user-agent": "SouthernStar/0.3.7 (private portfolio valuation; contact via account owner)",
