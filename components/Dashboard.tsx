...
export default function Dashboard() {
  ...
  return <main className="shell">
    ...
    {loading ? <div className="card empty">Loading NorthStar…</div> : <>
      ...
      {!data.totalValue && <section className="card onboarding"><div className="value">Start with your real data</div><p>Import the Directshares holdings CSV and IBKR Flex XML, add Macquarie cash, then record any physical platinum. The dashboard will update immediately and remain separated by legal owner.</p><div className="buttonRow"><Link className="button primary" href="/imports">Import broker data</Link><Link className="button" href="/cash">Add cash position</Link><Link className="button" href="/assets">Add physical platinum</Link></div></section>}
      ...
      {data.provisionalValue > 0 && <section className="card notice" style={{ marginTop: 16 }}><strong>{money(data.provisionalValue)} is provisional IBKR value</strong><p>The Trades-only Flex file establishes quantities and remaining cost basis. NorthStar is deliberately using cost basis until an IBKR Open Positions section or a live price provider supplies current market values.</p></section>}
